          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" label="姓名" name="full_name">
                            <uni-easyinput type="text" v-model="form['full_name']" v-if="user_group === '管理员' || (form['resident_users_id'] && $check_field('set','full_name')) || (!form['resident_users_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','room_number') || $check_field('add','room_number') || $check_field('set','room_number')" label="房号" name="room_number">
                            <uni-easyinput type="text" v-model="form['room_number']" v-if="user_group === '管理员' || (form['resident_users_id'] && $check_field('set','room_number')) || (!form['resident_users_id'] && $check_field('add','room_number'))" :disabled="disabledObj['room_number_isDisabled']" />
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "resident_users",
      is_password: true,
      field: "resident_users_id",
      url_add: "~/api/resident_users/add?",
      url_set: "~/api/resident_users/set?",
      url_get_obj: "~/api/resident_users/get_obj?",
      url_upload: "~/api/resident_users/upload?",
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/resident_users/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["resident_users_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?resident_users_id=" + form_sub["resident_users_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/resident_users/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/resident_users/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/resident_users/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/resident_users/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/resident_users/view','get');
