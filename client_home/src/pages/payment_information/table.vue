            ...
            <view class="">
              ...
              <uni-forms :modelValue="query">
                    <uni-forms-item label="月份" name="month">
                      <uni-easyinput type="text" v-model="query.month" placeholder="月份" />
                    </uni-forms-item>
                    <uni-forms-item label="房号" name="room_number">
                      <uni-easyinput type="text" v-model="query.room_number" placeholder="房号" />
                    </uni-forms-item>
                    <uni-forms-item label="缴费名称" name="payment_name">
                      <uni-easyinput type="text" v-model="query.payment_name" placeholder="缴费名称" />
                    </uni-forms-item>
                    </uni-forms>
              <!-- /搜索栏 -->
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="0 && $check_field('get','owner')" class="col">
						<view class="view">
                            ...
                            <span>业主</span>
								...
								<text>
									{{ get_user_owner(o['owner']) }}
													...
													<view v-if="1 && $check_field('get','month')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>月份</span>
                          </view>
                            ...
                            <span>房号</span>
								...
								<text>
									{{ o["room_number"] }}
													...
													<view v-if="1 && $check_field('get','payment_name')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>缴费名称</span>
                          </view>
                  ...
                  <view class="col">
					...
					<navigator  :url="'/pages/payment_information/view?' + field + '=' + o[field]"
	                    ...
	                    <a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/payment_information/table')" class='btn btn-primary'>支付</a>
	        					</view>
				</view>
			</view>
		</view>
...
<script>
    ...
    return param;
        ...
        async get_list_user_owner() {
          ...
          goto_pay(v) {
            //打开选择支付方式
            this.pay_obj.id = v.payment_information_id
            this.model=true;
          },
          choosepayway(idx){
            switch(idx){
          ...
          submit_pay(){
            let _this = this;
            let url = this.$toUrl(this.query, "~/api/payment_information/set?payment_information_id="+this.pay_obj.id);
            let param = {"pay_state":"已支付","pay_type":this.pay_obj.payActiveName}
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/payment_information/table");
                }, 700);
              }else if(res.error){
